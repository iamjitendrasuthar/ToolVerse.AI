...
export default function TrendingClient({
  ...
  return (
    <main className="min-h-screen bg-[#fafcff] text-slate-900 overflow-hidden selection:bg-orange-200 selection:text-orange-900 pb-20">
      {/* Ambient Background - Adjusted for a "Hot/Trending" vibe */}
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-0 left-1/4 w-[300px] md:w-[500px] h-[300px] md:h-[500px] bg-orange-200/20 blur-[80px] md:blur-[120px] rounded-full" />
        <div className="absolute bottom-0 right-1/4 w-[250px] md:w-[400px] h-[250px] md:h-[400px] bg-red-200/10 blur-[70px] md:blur-[100px] rounded-full" />
      </div>
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-4 md:px-6 pt-30 md:pt-32">
        ...
        <header className="mb-10 md:mb-16">
          ...
          >
            <div className="max-w-2xl">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-orange-50 border border-orange-100 text-orange-600 font-bold text-xs uppercase tracking-wider mb-4 md:mb-5 shadow-sm">
            ...
            <div className="inline-flex items-center gap-3 bg-white p-1.5 md:p-2 rounded-2xl border border-slate-200 shadow-sm w-fit">
              <div className="bg-gradient-to-br from-orange-500 to-red-600 p-1.5 md:p-2 rounded-xl text-white shadow-inner">
                <TrendingUp size={18} />
              </div>
        ...
        <section className="sticky top-[70px] md:top-24 z-30 mb-8 md:mb-12">
          <div className="bg-white/80 backdrop-blur-2xl border border-white/50 shadow-xl shadow-orange-900/5 rounded-2xl md:rounded-[2rem] p-2 md:p-3 flex flex-col lg:flex-row items-center gap-3 md:gap-4">
            {/* Search */}
            <div className="relative w-full lg:max-w-md group">
              <Search
                className="absolute left-4 md:left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-orange-500 transition-colors"
                size={18}
              />
              ...
              <input
                ...
                className="w-full bg-slate-100/50 border-none rounded-xl md:rounded-2xl py-3 md:py-4 pl-11 md:pl-14 pr-4 focus:ring-2 focus:ring-orange-500/20 text-sm md:text-base text-slate-800 font-medium transition-all outline-none"
                value={searchQuery}
            ...
            <div className="relative w-full lg:w-auto lg:ml-auto">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="p-1.5 bg-orange-50 rounded-lg text-orange-600 group-hover:bg-orange-500 group-hover:text-white transition-colors">
              ...
              >
                {filteredAndSortedTools.map((tool: any, index: number) => (
                  ...
                  >
                    <Link
                      href={`/tool/${tool.slug}`}
                      className="group flex flex-col h-full bg-white border border-slate-200/80 rounded-[1.25rem] overflow-hidden hover:border-orange-200 hover:shadow-[0_15px_30px_rgba(249,115,22,0.08)] hover:-translate-y-1 transition-all duration-500 relative"
                    >
                      {/* Trending Rank Badge */}
                      <div className="absolute top-2 left-2 z-30 w-7 h-7 bg-white/90 backdrop-blur-md rounded-full flex items-center justify-center shadow-sm border border-orange-100 font-black text-xs text-orange-600">
                        #{index + 1}
                      </div>

                      <button
                        className="absolute top-2 right-2 z-30 p-1.5 bg-white/80 backdrop-blur-md rounded-full text-slate-400 hover:text-orange-500 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all shadow-sm cursor-pointer"
                        onClick={(e) => {
                          e.preventDefault();
                          e.stopPropagation();
                        }}
                      ...
                      <div className="px-3 pt-6 pb-4 flex flex-col flex-grow relative bg-white z-20">
                        ...
                        <div className="flex justify-between items-start gap-1 mt-1 mb-1.5">
                          <h3 className="text-sm font-bold tracking-tight text-slate-900 group-hover:text-orange-600 transition-colors line-clamp-1">
                            {tool.name}
                          </h3>
                          {tool.rating >= 4.8 && (
                            <CheckCircle2
                              size={12}
                              className="text-orange-500 fill-orange-50 shrink-0"
                            />
                          )}
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-3 border-t border-slate-100 mt-auto">
                          ...
                          <div className="flex items-center text-xs sm:text-sm font-bold text-slate-400 group-hover:text-orange-600 transition-colors">
                            Explore
                            <ArrowUpRight
                              size={14}
